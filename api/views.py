...
class LeaveRoom(APIView):
    def post(self, request, format=None):
        if 'room_code' in self.request.session:
            ...
            qs = Room.objects.filter(host=host_id)
            if qs.exists():
                room = qs.first()
                room.delete()
...
class UpdateRoom(APIView):
    ...
    def patch(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if qs.exists():
                room_object = qs.first()
                user_id = self.request.session.session_key
                if room_object.host != user_id:
                    return Response({'message': 'You are not the host of this room.'}, status=status.HTTP_403_FORBIDDEN)
                room_object.guest_can_pause = guest_can_pause
                room_object.votes_to_skip = votes_to_skip
                room_object.save(update_fields=['guest_can_pause', 'votes_to_skip'])
                return Response(RoomSerializer(room_object).data, status=status.HTTP_200_OK)
            else:
